...
// DefiTokenSupply staking tokens from the total supply
func (k Keeper) DefiTokenSupply(ctx sdk.Context) sdk.Int {
	return k.bankKeeper.GetSupply(ctx).GetTotal().AmountOf(k.DefiBondDenom(ctx))
}

// BondedRatio the fraction of the staking tokens which are currently bonded
func (k Keeper) BondedRatio(ctx sdk.Context) sdk.Dec {
	defiSupply := k.DefiTokenSupply(ctx)
	if defiSupply.IsPositive() {
		return k.TotalBondedTokens(ctx).ToDec().QuoInt(defiSupply)
	}
