...
func (res QueryDelegatorTotalRewardsResponse) String() string {
	...
	for _, reward := range res.Rewards {
		out += fmt.Sprintf(`  
	ValidatorAddress: %s
	Reward: %s`, reward.DefiAddress, reward.Reward)
	}
	out += fmt.Sprintf("\n  Total: %s\n", res.Total)
...
//nolint:interfacer
func NewDelegationDelegatorReward(defiAddr sdk.ValAddress,
	reward sdk.DecCoins) DelegationDelegatorReward {
	return DelegationDelegatorReward{DefiAddress: defiAddr.String(), Reward: reward}
}
