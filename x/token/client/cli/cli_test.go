...
import (
	...
	tokentestutil "github.com/gauss/gauss/v4/x/token/client/testutil"
	tokentypes "github.com/gauss/gauss/v4/x/token/types"
...
func (s *IntegrationTestSuite) TestToken() {
	...
	args := []string{
		...
		fmt.Sprintf("--%s=%d", tokencli.FlagDecimals, decimals),
		fmt.Sprintf("--%s=%d", tokencli.FlagInitialSupply, initialSupply),
		fmt.Sprintf("--%s=%d", tokencli.FlagTotalSupply, totalSupply),
	...
	s.Require().NoError(err)
	expectedParams := "{\"issue_token_fee\":{\"denom\":\"stake\",\"amount\":\"10000000\"},\"mint_token_fee_rate\":\"0.100000000000000000\"}"
	...
	newName := "Kitty Dog"
	newTotalSupply := 200000000
	newMintable := false
	args = []string{
		fmt.Sprintf("--%s=%s", tokencli.FlagName, newName),
		fmt.Sprintf("--%s=%d", tokencli.FlagTotalSupply, newTotalSupply),
